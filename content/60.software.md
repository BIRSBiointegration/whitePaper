...
One potential challenge in the analysis of multi-modal data is the different resolution of each modality. For instance in scNMT-seq, while gene expression has a natural unit (the gene), other modalities may be summarized at different genomic scales, e.g., gene promoters, exons, introns, or gene bodies. Furthermore, the observations of different modalities may not be directly comparable. As an example, different spatial transcriptomic protocols have different resolutions, ranging from multi-cellular to sub-cellular. This is a challenge when integrating spatial transcriptomics and single-cell RNA-seq. Finally, because of the absence of universal standards, the quantity and quality of available metadata may vary across datasets and modalities.
