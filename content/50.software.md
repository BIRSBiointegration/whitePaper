...
With increasing complexity and richness in our scientific communications, we use color as a powerful tool to represent this richness in most of our visualizations.
It is desirable to standardize visualizations to facilitate interpretability and accessibility with respect to color vision deficiency. Color vision deficiency could not just lead to difficulties in perceiving patterns in multi-colored figures (the basis for the Ishihara's colorblindness tests), but in rare cases, the perceived patterns could differ between individuals with normal and color deficient vision.
One strategy to address these issues is to include colorblind friendly visualizations as a default setting. Numerous colorblind-friendly palettes exist (see viridis, dittoSeq, etc.) which could be integrated into our visualizations as the default option. Even with these palettes in place, it is desirable to limit the number of colors in visualizations(~8). To reduce the dependence on colors, one solution would be to include additional visual cues to differentiate regions (hatched areas) or cells (point shapes). Beyond the issues of color vision deficiency, there needs to be a broader discussion regarding the accessibility of our figures with respect to a variety of visual impairments. (Perhaps authors can include an "accessibility caption" accompanying figures which "guide" the reader's perception of the images.)
