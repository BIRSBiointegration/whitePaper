...
<small><em>
This manuscript
([permalink](https://BIRSBiointegration.github.io/whitePaper/v/0360a075294907aaba326a9c97e0caace4cbe08d/))
was automatically generated
from [BIRSBiointegration/whitePaper@0360a07](https://github.com/BIRSBiointegration/whitePaper/tree/0360a075294907aaba326a9c97e0caace4cbe08d)
on August 14, 2020.
</em></small>
...
**A** Challenges 1 and 2: Defining genomic contexts, varying dimensions and different amount of missing values across modalities (**missing values to be added - CTCF will most likely be dropped**).
**B** Supervised analysis using integrative distance measure (MOSAIC). Methylation data are more predictive than chromatin accessibility during gastrulation **Arshi: the acc data should be excluded**.
**C** (LIGER) Unsupervised analysis using NMF to extract common and view-specific variation (LIGER) **Josh: legend needs to be fixed**.
**D** (PLS) Unsupervised integration characterizes common variation with respect to RNA. The concordant axes of variation separate different embyonic stages. Various modalities differ in amount of covariation with transcriptome **Al: CTCF should be dropped - measure of concordance can be added to subtitles**.
**Ricard and Al: consider a separate table for performance measures (concordance/alignment/MI)**.
...
We'll separate out the different challenges into different levels. **Elana - expand this to flow into the next subsections - I moved the language to the end as more of a summary to flow better from the previous section. I could also see an arguemnt for keeping it as was but instead of stating it as "the first challenge" say "part of the source of the numerous disparate algorithms employed in the analyses in the hackathons is the biology, but an equal factor is the disparate namings for similar classes of algorithms between computational disciplines. I'm open and happy to discuss either approach **
