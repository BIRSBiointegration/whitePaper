...
<small><em>
This manuscript
([permalink](https://BIRSBiointegration.github.io/whitePaper/v/7e49dea13f553588316e9cf5f00571ae18e29c17/))
was automatically generated
from [BIRSBiointegration/whitePaper@7e49dea](https://github.com/BIRSBiointegration/whitePaper/tree/7e49dea13f553588316e9cf5f00571ae18e29c17)
on August 24, 2020.
</em></small>
...
**B** Assessment of cell type prediction using different data normalizations and classifiers. Normalization strategies included none (raw), counts per million (cpm), ComBat batch correction applied to cpm (cpm_combat), scRNAseq and seqFISH scaled using the first eigenvalue (cpm_eigen), latent variables retained for both datasets after applying partial least squares to cpm_eigen normalized data (cpm_pls). A supervised multinomial classifier with an elastic net penalty (enet) was trained using the scRNAseq data and cell-type labels (provided). Each model was then applied to the seqFISH data to determine the predicted cell-type labels. A semi-supervised multinomial classifier with an elastic net penalty (ssenet) was trained using the scRNAseq and seqFISH data and cell-type labels (provided) to impute the "missing" cell-types labels of the seqFISH dataset. The gower distance between each method-normalization pair was computed (including SVM-based predictions from [@doi:10.1038/nbt.4260]). The multidimensional scaling plot depicts the similarity between different methods and normalizations based on predicted cell-type labels. The first dimension (Dim1) separates methods that normalize the scRNAseq and seqFISH data together (dashed) and separately (solid). The normalization method used had a stronger impact on cell-type predictions than the classification method used.
...
During the hackathon we tested various machine learning or data integration models, but also noted that initial dataset normalization strategies might have a significant impact on the final results (see (Figure {@fig:spatial}B).
...
The hackathon participants selected two machine learning classifiers, a supervised support vector machine (SVM) and, a supervised and semi-supervised generalized linear model regularized with an elastic net penalty (enet and ssenet).
...
The ssenet approach used a iterative model building approach, which combines both datasets and initially only retains the highest confidence labels and then gradually adds more cell type labels until all cells have been classified.
...
Gastrulation marks a major lineage specification event in mammalian embryos, accompanied by profound transcriptional rewiring and epigenetic remodelling [@doi:10.1093/humupd/dmy021].

In this hackaton we used a data set where scNMT-seq was applied to study mouse gastrulation [@doi:10.1038/s41586-019-1825-8] by profiling cells across four developmental stages (E4.5 to E7.5). For simplicity, in the workshop we focused on the integration of RNA expression and DNA methylation, quantified over the following genomic contexts: gene bodies, promoters, CpG islands, and DHS open sites. A total of 799 cells passed quality control for the two modalities. Overview of data is shown in Figure {@fig:scnmteq}A. 
...
<!--The three hackathon analyses emphasized that regardless of the common challenges faced by our participants, there is no one method fits all. Methods derived from bulk RNA-seq are generally not sufficient for single cell data and face further challenges when the pertinent molecular features cannot be summarized into simple gene level statsistics. Therefore, further developments are needed to fully suit these emerging data. It is important that the challenges highlighted in this section are fully solved to enable subsequent interpretation of multi-scale biological systems from multi-omics data. Results interpretation of these analyses, however, raise further challenges.--!>
...
**(Elana) expand this to flow into the next subsections - I moved the language to the end as more of a summary to flow better from the previous section. I could also see an arguemnt for keeping it as was but instead of stating it as "the first challenge" say "part of the source of the numerous disparate algorithms employed in the analyses in the hackathons is the biology, but an equal factor is the disparate namings for similar classes of algorithms between computational disciplines. I'm open and happy to discuss either approach **
...
Community-developed, open source software and data sharing platforms enable the application of novel computational frameworks to multimodal genome-scale experiments.  Our hackathons were designed to leverage and build on these open frameworks.  We provided easy to use data packages to distribute the multi-omics data, and we developed vignettes that reproducibly demonstrate each of the analysis approaches employed for these datasets (https://github.com/BIRSBiointegration/Hackathon). These open-source hackathon components provide a critical resource to enable the use and evaluation of robust benchmark strategies, and to support the enhancement and extension of the components themselves. Data structures and vignettes were largely developed on the R/Bioconductor ecosystem for reproducible, open-source, open development genomic analysis. In this section, we reflect on the software infrastructure that enabled the analytic frameworks for inference and visualization of the multimodal single-cell experiments in the hackathons.  Additionally, we discuss unmet challenges identified through the hackathons, that require future development. A wide array of open-source genomics frameworks for multi-platform single cell data are also developing in R, Python, and in other software frameworks that enable graphical user interfaces for point-and-click exploration. Exploration and engagement with all these tools is richly enabled through code sharing resources like GitHub.
